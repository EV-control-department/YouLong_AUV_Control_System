...
class BasicMotionNode(Node):
    ...
    def start(self):
        ...
        self.get_logger().info(
            ...
            f'{self._origin.y:.2f}), yaw={self._origin.rz:.1f}°')
    ...
    def _body_to_odom(self, pos: Coordinate) -> Coordinate:
        """body 坐标 → odom 坐标（以当前 pose 为原点）。"""
        return self.pose.to_world_frame(pos)   
    
    def _odom_to_body(self, pos: Coordinate) -> Coordinate:
        """odom 坐标 → body 坐标（以当前 pose 为原点）。"""
        return self.pose.to_local_frame(pos)
    ...
    def _calc_step_size(self, move_angle_rad: float) -> float:
        """根据运动方向计算基础步进距离，这里的运动方向应该是指相对于机体坐标系的运动方向。

        步长 = |cos(θ)| * TOL_X + |sin(θ)| * TOL_Y

        纯 X 移动 (θ=0°): step = 0.1m
        纯 Y 移动 (θ=90°): step = 0.1m
        斜向 45°: step ≈ 0.14m
        """
    ...
    def _action_goal_cb(self, _goal_request):
        return GoalResponse.ACCEPT

    def _action_cancel_cb(self, _goal_handle):
        self.get_logger().info('BasicMotion action cancelled')
    ...
    def _action_execute_cb(self, goal_handle):
        ...
        result.message = "" if success else "motion timeout"
        if success:
            goal_handle.succeed()
        else:
            goal_handle.abort()
