...
def _physical_key(message) -> tuple[str, int]:
    """Use the localizer's physical identity, not the raw detector label."""
    physical_name = str(getattr(message, "physical_class_name", "")).strip()
    return (physical_name or str(message.class_name), int(message.instance_id))
...
class TargetPositionGui(Node):
    ...
    def __init__(self):
        ...
        self.targets: dict[tuple[str, int], TargetPosition] = {}
    ...
    def _build_window(self):
        ...
        ttk.Label(
            toolbar,
            text="N-E：滚轮缩放、左键拖拽；蓝=前视双目；橙=前视多视角；绿=下视直接",
        ).pack(side=tk.RIGHT)
    ...
    def _build_target_table(self, parent):
        frame = ttk.Labelframe(parent, text="融合目标（/perception/target_positions）", padding=5)
        frame.pack(fill=tk.BOTH, expand=False)
        columns = ("id", "class", "x", "y", "z", "sigma", "forms", "status", "age")
        self.target_tree = ttk.Treeview(
            frame, columns=columns, show="headings", height=10, selectmode="browse")
        headings = {
            "id": "物理类/实例", "class": "检测类别", "x": "X 北", "y": "Y 东",
            "z": "Z 下", "sigma": "σ合成", "forms": "已融合观测",
            "status": "状态", "age": "年龄(s)",
        }
        widths = {
            "id": 140, "class": 175, "x": 65, "y": 65, "z": 65,
    ...
    def _refresh(self):
        ...
        self.status_text.set(
            f"融合目标：{len(self.targets)} 个；几何观测：{len(self.observations)} 条；"
            f"目标消息：{self.target_stamp}；观测消息：{self.observation_stamp}"
        )

    def _visible_key(self, key: tuple[str, int]) -> bool:
    ...
    def _refresh_observation_table(self):
        ...
        for observation in history[:self.observation_table_limit]:
            ...
            values = (
                ...
                f"{key[0]}/{key[1]}", observation.class_name,
    ...
    def _select_target(self, _event):
        ...
        if not selected:
            return
        try:
            physical_class_name, instance_id = selected[0].split(":", maxsplit=1)
            selected_key = physical_class_name, int(instance_id)
        except ValueError:
            return
    ...
    def _draw_scene(self):
        ...
        auto_bounds = self._scene_bounds(targets, observations)
        bounds = (
            self._xy_view_bounds
            if self.view_plane.get() == "xy"
        ...
        for target in targets:
            ...
            x, y = canvas_point(point_x, point_y)
            color_seed = sum(ord(char) for char in key[0])
            color = TARGET_COLORS[(color_seed * 31 + key[1]) % len(TARGET_COLORS)]
            covariance = target.position_covariance
            ...
            radius = 7 if key == self.selected_key else 5
            self.canvas.create_oval(
                x - radius, y - radius, x + radius, y + radius,
    ...
    def _scene_bounds(self, targets, observations):
        points = []
        for target in targets:
