...
- **vision** — YOLO 目标检测 + YOLO-Seg 管道分割，将双目拼接图拆分为左右独立通道分别推理
- **position** — 单目多帧射线交会，从 2D 检测结果反算 3D 世界坐标

两个节点通过 `DetectionArray` 消息解耦：vision 只关心"图像中有什么"，position 只关心"目标在 3D 空间中的位置"。管道巡线（LineState）则直接由 vision 发布，供 task_runner 的 LineFollower 子对象消费。
...
down_cam/right ─┘                            │                              │
                                             ...
                                             └─┤                              │
                                                └─ down_right (1280×960) ──→ /perception/detection/down_right
                                                                             │
                                                ┌─ down_left  ── YOLO-Seg ──→ /perception/line/down_left
                                                └─ down_right ── YOLO-Seg ──→ /perception/line/down_right
                                                                             │
...
| `/perception/line/down_left` | `LineState` | 下视左目管道分割结果 |
| `/perception/line/down_right` | `LineState` | 下视右目管道分割结果 |
...
| `/perception/aruco/ids` | `std_msgs/Int32MultiArray` | 前视 ArUco 检测到的 marker ID 列表 (ID 1-6) |

### 参数

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `model_path` | `str` | `""` | YOLO 检测模型 .pt 文件路径。为空时自动搜索默认路径 |
| `segment_model_path` | `str` | `""` | YOLO-Seg 分割模型 .pt 文件路径（管道巡线用）。为空时不启用管道分割 |
| `publish_annotated` | `bool` | `false` | 是否发布 YOLO 标注图像（带检测框） |
...
当 `segment_model_path` 指向有效的 YOLO-Seg 模型时，vision 对下视双目图像额外运行分割推理，提取管道 mask 并发布 `LineState` 消息。

**处理流程：**

```
下视双目图像 → YOLO-Seg 推理 → 分割 mask
  → cv2.findContours 提取轮廓
  → cv2.moments 计算质心 → center_error
  → cv2.minAreaRect 拟合方向 → heading_error_deg
  → ScalarKalman 滤波平滑 (Q=1.0, R=3.0)
  → 发布 /perception/line/down_{left,right}
```

**置信度过滤：** YOLO-Seg 置信度阈值 0.9，低于此值的结果被丢弃。

**Kalman 滤波器：** 1D 标量 Kalman，center 和 heading 各一个独立滤波器，用于平滑连续帧间的噪声。
...
|------|------|
| `detected` | 是否检测到管道 |
| `center_error` | 管道中心与图像中心的归一化偏移 [-1, 1]，负=偏左，正=偏右 |
| `heading_error_deg` | 管道长轴与竖直方向夹角 [-90°, 90°]，正=顺时针偏 |
| `area_ratio` | 分割 mask 面积 / 图像面积，反映管道在画面中的占比 |

**下游消费者：** `LineFollower` (uv_task/line_follower.py) 订阅左右下视 LineState，融合后驱动双通道 PID 巡线控制。
...
vision 节点启动一个后台线程（~20Hz），从前视相机的左右半幅图像中检测 ArUco 标记：

- 字典：`cv2.aruco.DICT_4X4_50`
- 输入：`/auv/front_cam/stitched`（2560×960 拼接图）→ 拆分后对左右半幅分别检测
- 输出：`/perception/aruco/ids`（`std_msgs/Int32MultiArray`），仅包含 ID 1-6
- 线程安全：前端画面通过 `threading.Lock()` 保护共享帧

此功能为 `arrow_surface` 任务的扇区选择提供依据（ArUco ID 1,2→yellow, 3,4→green, 5,6→red）。
...
| `LineFollower` | `uv_task` | 订阅 `/perception/line/down_*` 和 `/perception/detection/down_*`，视觉伺服巡线 |
