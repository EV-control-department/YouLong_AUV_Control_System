...
| `arrow_surface` | —       | `{timeout, ...}` | 箭头对准+扇区对准+投球 (ArrowSurfacer) |
...
```bash
ros2 service call /task/exec uv_msgs/srv/ExecTask \
  "{task_name: 'arrow_surface', params_json: '{\"timeout\": 300, \"sector_x\": 5, \"sector_y\": 6}', timeout: 0.0}"
```

**参数（tasks.json 中 `arrow_surface` 的 params）：**

| 参数 | 默认值 | 说明 |
|---|---|---|
| `arrow_class_id` | 3 | 箭头的 YOLO class_id |
...
| 步骤 | 动作 | 说明 |
|---|---|---|
| 1 | setrz → view_yaw | 转向搜索方向 |
| 2 | 微步螺旋搜索 | 下视 YOLO 搜索箭头 class_id |
| 3 | 100 次 SET xy | 双目三角测量接近箭头 |
| 4 | WMOVE z=-1 | 短暂出水 (ArUco 读取) |
| 5 | WMOVE z=0.4 | 恢复深度 |
| 6 | 前视 ArUco 扫描 | 无 ID 时 ±15° 摇头 + 下潜上升 0.5m |
| 7 | WTRAVEL | 前往扇区位置 (sector_x, sector_y) |
| 8 | 搜索+接近扇区 | 下视 YOLO + 100 次双目接近 |
| 9 | 投球 (INFO) | 日志占位 |

**日志关键字：**

- `ArrowSurfacer created` — 子对象初始化
- `ArrowSurfacer: arrow found during search!` — 搜索成功
- `ArrowSurfacer: approach #N` — 接近迭代
- `ArrowSurfacer: surfacing (setz=-1)` — 出水
- `ArrowSurfacer: ArUco marker #N detected` — 检测到 marker
- `ArUco frontal search exhausted` — 前视扫描无结果（将默认绿色）
- `throwing ball to <color> sector` — 投球占位
- `ArrowSurfacer destroyed` — 清理完成
