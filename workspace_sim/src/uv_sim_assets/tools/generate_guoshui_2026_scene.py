...
GUOSHUI_OBJECTS = ASSET_ROOT / "objects" / "guoshui_2026"
...
def generate(seed: int, template: Path, output: Path) -> dict[str, object]:
    ...
    split_collection_net(
        GUOSHUI_OBJECTS / COLLECTION_NET_SOURCE,
        GUOSHUI_OBJECTS / COLLECTION_NET_SIDE,
        GUOSHUI_OBJECTS / COLLECTION_NET_BOTTOM,
    )
    ...
    for scene_name, mesh_name in SEEDED_GATE_MODELS.items():
        ...
        mesh.attrib["filename"] = f"objects/guoshui_2026/{mesh_name}"
...
def main() -> None:
    data_dir = ASSET_ROOT / "worlds" / "guoshui_2026"
