...
左侧默认是 `odom` 坐标系中的 N-E 俯视平面（项目约定仍为 X=North、Y=East；单位米），但显示方向固定为**上方 North、右方 East**，以与赛场俯视图一致；可在顶部切换为 X-Z 或 Y-Z 侧视图，以检查 Z=Down。图中的青色轮廓是机器人当前位置模型，红色箭头表示机头方向；模型按 `/basic_motion/pose_info` 的实时 X/Y/Z 和 Roll/Pitch/Yaw 更新，右上角同时显示数值姿态。当前 `object_localizer` 同时发布独立的前视和下视估计：
...
- 蓝色菱形：前视估计（前视双目点 + 前视多位置交会点 → 前视池 → K 聚类 → 50 条滤波队列）；
- 绿色圆形：下视估计（下视双目点 → 下视池 → K 聚类 → 50 条滤波队列）；
- 虚线椭圆：对应来源估计在当前显示平面内约 2σ 不确定度范围；
- 灰色虚线及 `Δ=...m`：同一物理类别按水平位置最近匹配的前视/下视估计之间的三维距离，用来直接观察两套结果差多远；
- 蓝色实线点：前视双目三维观测；橙色虚线箭头：前视多位置单目射线；绿色点：下视直接三维观测。

右上表展示所有前视/下视估计；`估计来源` 列明确是前视还是下视，不能只看物理类别/实例。检测类别列显示代表原始标签及本来源轨迹已经接受的 YOLO ID。`已融合观测` 是位掩码组合，前视双目=1、前视多视角=2、下视直接=4。右下表逐条列出观测序号、物理类别/实例、该帧的原始检测类别、观测形式、测量点或射线原点、置信度与时间。点击任意目标或观测行会高亮对应来源实例；勾选“仅显示选中目标”可以排查错误关联。

定位器的聚类数量先验为：引导线 K=6、门 K=4、其余物品每类 K=1；前视和下视各自执行一遍。`*_front`/`*_down` 后缀会归一到物理类别，观测历史中的 `class_id` 则始终是该帧检测模型实际输出的 ID。观测先进入来源内类别池，再按二维 N/E 聚类；每个来源内实例最多保留 50 条卡尔曼队列，类别观测池默认保留 2000 条。Z/Down 不参与实例空间聚类，因此深度异常不会单独制造第二个实例。
